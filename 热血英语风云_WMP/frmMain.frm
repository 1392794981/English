...
Begin VB.Form frmMain 
   ...
   Begin MSComctlLib.ListView lvwLesson 
      ...
      NumItems        =   0
   End
   Begin WMPLibCtl.WindowsMediaPlayer wmpRead 
      Height          =   375
      Left            =   9120
      TabIndex        =   54
      Top             =   6240
...
'    mciRead.Command = "play"
    wmpRead.URL = strFileName
    wmpRead.settings.setMode "loop", False
    wmpRead.Controls.currentPosition = GetCurrentDividePoint
    wmpRead_To = GetNextDividePoint
    wmpRead.Controls.play
    
    If chkAddTime.Value = Checked Then
        'txtPauseTime.Text = CStr(Val(txtAddTime.Text) + ((mciRead.To - mciRead.From) \ 1000))
        txtPauseTime.Text = CStr(Val(txtAddTime.Text) + ((GetNextDividePoint - GetCurrentDividePoint)))
    End If
End Sub
...
Private Sub cmdCarefulListen_Click()
    ...
    Call EnglishPlay(GetCurrentDividePoint, rate)
...
'        frmMain.mciRead.Command = "stop"
        ...
        If WindowsMediaPlayer1.URL <> lvwDividePoint.SelectedItem.SubItems(2) Then
...
Private Sub cmdLoadLrc_Click()
    ...
    If Dir(strFileName) <> "" Then '文件存在
        ...
        Do While Not EOF(1)
            ...
            If intColon > intStart And intEnd > intColon Then
            
                dblMinutes = Val(Mid(str, intStart + 1, intColon - intStart - 1))
                dblSecond = Val(Mid(str, intColon + 1, intEnd - intColon - 1))
                dblSecond = (dblMinutes * 60 + dblSecond)
                
                '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                If dblSecond > 0 Then
                    Dim myListItem As ListItem
                    Set myListItem = lvwDividePoint.ListItems.Add
                    myListItem.Text = Format(dblSecond, "000000.000")
                                
                    Seconds = (dblSecond) Mod 60
                    Minutes = (dblSecond \ 60) Mod 60
                    Hours = (dblSecond \ 3600)
...
Private Sub mnuClearItems_Click()
...
Private Sub Timer1_Timer()
    If blShowCarefullListenForm = True Then
        ...
        If WindowsMediaPlayer1.Controls.currentPosition >= (PositionTo - 0.1) Then
        ...
        frmCarefullListen.ProgressBar1.Max = Int((PositionTo - PositionFrom) * 1000)
        frmCarefullListen.ProgressBar1.Value = Int((WindowsMediaPlayer1.Controls.currentPosition - PositionFrom) * 1000)
    End If
End Sub
...
'    End If
    ...
    If wmpRead.playState = wmppsPlaying Then
        wmpRead.Controls.pause
    Else
        wmpRead.Controls.play
    End If
...
'    mciRead.Command = "play"
    ...
    sldSound.Min = 0
    sldSound.Max = getLength 'mciRead.Length
    
    lvwDividePoint.ListItems.Clear
    Call cmdPlay2_Click
End Sub

Private Function getLength() As Long
    'Sleep 1000
'    Dim i
'    For i = 0 To 10
'        MsgBox "openstate:" + CStr(wmpRead.openState) + "playstate:" + CStr(wmpRead.playState) + "status:" + CStr(wmpRead.Status) + " duration:" + CStr(wmpRead.currentMedia.duration)
'    Next
    ...
    For i = 0 To 1000
        DoEvents
        If wmpRead.playState <> 9 Then ' wmppsReady Or wmpRead.playState = wmppsPlaying Then
            'Exit Do
            Exit For
        End If
        Sleep 1
    Next i
    'Loop
    getLength = wmpRead.currentMedia.duration
    'MsgBox CStr(getLength)
End Function
...
Private Sub cmdPlay2_Click()
    ...
    'MsgBox CStr(wmpRead.Controls.currentPosition)
    myListItem.Text = Format(wmpRead.Controls.currentPosition, "000000.000")
...
Private Sub cmdTract_Click()
    ...
    intCurrentPosition = wmpRead.Controls.currentPosition
    strFileName = lvwDividePoint.SelectedItem.SubItems(2)
...
Private Sub Form_Load()
    ...
    lvwDividePoint.ColumnHeaders.Add , , "断点", 2000
    ...
    Select Case Key
        ...
        Case "Alt_Divide"
            If blShowCarefullListenForm = True Then
                ...
                Else
                    Call EnglishPlay(GetCurrentDividePoint(), Val(txtRate.Text))
                End If
            End If
        Case "Alt_Multiply"
            If blShowCarefullListenForm = True Then
                ...
                If pos + 2 < WindowsMediaPlayer1.currentMedia.duration Then
                    Call EnglishPlay(pos + 2, Val(txtRate.Text))
                Else
                    Call EnglishPlay(WindowsMediaPlayer1.currentMedia.duration - 0.01, Val(txtRate.Text))
                End If
            End If
        Case "Divide"
            'Call JumpPlay(mciRead.Position - 5000)
            
            Dim prePosition As Double
            prePosition = wmpRead.Controls.currentPosition - 5
            Call JumpPlay(prePosition)
        Case "Multiply"
            'Call JumpPlay(mciRead.Position + 5000)
            Call JumpPlay(wmpRead.Controls.currentPosition + 5)
        Case "PageDown"
            SendMessage lvwDividePoint.hWnd, WM_KEYDOWN, VK_DOWN, 0
...
Private Sub mnuOpen_Click()
    ...
    If Len(strDialog_File_Name) = 0 Then Exit Sub
...
Private Sub tmrPlay_Timer()
    ...
    sldSound.Value = wmpRead.Controls.currentPosition
    
    On Error GoTo ErrHandler
    'If mciRead.Position >= mciRead.To Then
     If wmpRead.Controls.currentPosition >= wmpRead_To Then
        If chkPauseTime.Value = Checked Then
...
Private Function GetPlayTime() As String
    ...
    'dblSecond = mciRead.Position
    dblSecond = wmpRead.Controls.currentPosition
    Seconds = (dblSecond \ 1) Mod 60
    Minutes = (dblSecond \ 60) Mod 60
    Hours = (dblSecond \ 3600)
...
Private Function PositionToString(dblSecond As Double) As String
    Dim Hours As Integer
    Dim Minutes As Integer
    Dim Seconds As Integer
...
Public Function GetCurrentDividePoint() As Double
    
    If lvwDividePoint.ListItems.Count < 1 Then Exit Function
    
    GetCurrentDividePoint = Val(lvwDividePoint.SelectedItem.Text)
    'GetCurrentDividePoint = GetCurrentDividePoint / 1000
    
End Function

Public Function GetNextDividePoint() As Double
    ...
    If lvwDividePoint.SelectedItem.Index < lvwDividePoint.ListItems.Count Then
        ...
        'GetNextDividePoint = GetNextDividePoint / 1000
    Else
        GetNextDividePoint = getLength() ' mciRead.Length
    End If
End Function
...
'    mciRead.Command = "play"
    wmpRead.Controls.currentPosition = sldSound.Value
    wmpRead.Controls.play
    blMoveSlider = False
End Sub
...
Private Sub mnuEditDelAllDividePoint_Click()
    ...
    Dim myListItem As ListItem
    Set myListItem = lvwDividePoint.ListItems.Add
    myListItem.Text = Format(0, "000000.000")
    myListItem.SubItems(1) = PositionToString(0)
    myListItem.SubItems(2) = strFileName
End Sub

Private Sub mnuEditDelDividePoint_Click()
    If Val(lvwDividePoint.SelectedItem.Text) < 1 Then
        Exit Sub
    End If
...
Private Sub JumpPlay(ByVal varFrom As Long)
    If varFrom < 0 Then
        wmpRead.Controls.currentPosition = 0
    ElseIf varFrom > wmpRead.currentMedia.duration Then
        wmpRead.Controls.currentPosition = wmpRead.currentMedia.duration
    Else
        'mciRead.From = varFrom
        wmpRead.Controls.currentPosition = varFrom
    End If
    'mciRead.Command = "play"
    wmpRead.Controls.play
End Sub
